var uno = {
	...
	render: function() {
		...
		for ( var i=1;i<=this.numOfPlayer;i++ ) {
			$("<section />").addClass("hand player"+i).appendTo(".players");
		}
	...
	passCards: function() {
		for ( var i=1;i<=this.numOfPlayer;i++ ) {
			for ( var k=1;k<=this.numOfCards;k++ ) {
	...
	swapDecks: function() {
		...
		// Resort
		for ( var i=0;i<gamedeck.length-1;i++ ) {
	...
	myTurn: function() {
		$("body")
			...
			.delegate("#carddeck","click",function() {
				if ( uno.playerTurn == 1 ) {
					uno.pass();
				}
			})
			.delegate("section.player1 .card","click",function() {
				if ( uno.playerTurn === 1 ) {
					if ( uno.rules($(this)) ) {
						uno.go( $(this) );
					}
